...
class QwenLVAR(nn.Module):
    ...
    def get_projected_image_tokens(self, batch: Dict[str, Any]) -> torch.Tensor:
        """
        Extract LM-space post-merge projected image tokens from Qwen2-VL visual encoder.

        The returned tokens must match the number of image placeholder tokens in
        the language prompt, i.e. the post-spatial-merge grid, not the raw
        pre-merge vision patch grid.
        """
        ...
        # Normalize possible HF output containers.
        if hasattr(image_tokens, "last_hidden_state"):
            image_tokens = image_tokens.last_hidden_state
        elif hasattr(image_tokens, "hidden_states") and image_tokens.hidden_states is not None:
            image_tokens = image_tokens.hidden_states[-1]
        elif isinstance(image_tokens, (tuple, list)):
            image_tokens = image_tokens[0]

        if image_tokens.dim() == 3:
            image_tokens = image_tokens[0]

        # Some visual wrappers expose pre-merger tokens. Convert them to the
        # post-merge/projected LM-space tokens expected by the text prompt.
        if self._current_premerge_grid is not None and self._current_postmerge_grid is not None:
            pre_expected = self._current_premerge_grid[0] * self._current_premerge_grid[1]
            post_expected = self._current_postmerge_grid[0] * self._current_postmerge_grid[1]

            if image_tokens.size(0) == pre_expected and image_tokens.size(0) != post_expected:
                merger = getattr(visual, "merger", None)
                if merger is None:
                    raise ValueError(
                        "Visual encoder returned pre-merge tokens, but no visual.merger was found "
                        "to project them into post-merge LM-space tokens."
                    )
                image_tokens = merger(image_tokens)

            if image_tokens.size(0) != post_expected:
                raise ValueError(
                    f"Expected {post_expected} post-merge tokens for grid {self._current_postmerge_grid}, "
